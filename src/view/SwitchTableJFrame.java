 ...
 * @author omar
 */
public class SwitchTableJFrame extends javax.swing.JFrame {
    ...
    public SwitchTableJFrame(POS sell, String nameTable, String nameArea, int tableId, int rowCount) {
        initComponents();
        this.setLocationRelativeTo(null);// center form in the screen
        this.sell = sell;
    ...
    public void loadInformationTable() {
        ...
        List<CoffeeTable> coffeeTableNotSelectedTable = cti.getAllCoffeeTablesWithoutSelectedTable(findArea.getAreaId(), tableId);
        CoffeeTable coffeeTableByTableId = cti.getCoffeeTableByTableId(tableId);
        if (coffeeTableByTableId.getStatus() == 1) {
            for (CoffeeTable c : coffeeTableNotSelectedTable) {
                if (c.getStatus() == 0) {
    ...
    private void initComponents() {
        ...
        jPanel2Layout.setVerticalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel2Layout.createSequentialGroup()
                ...
                .addComponent(switchTable, javax.swing.GroupLayout.PREFERRED_SIZE, 38, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addGap(37, 37, 37)
    ...
    private void switchTableActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_switchTableActionPerformed
        ...
        if (selectedTable != null) {
            if (rowCount > 0) {
                boolean updateTableIdTableProduct = tpi.updateTableIdTableProduct(tableId, selectedTable.getTableId());
            }
            boolean updateTableIdOrderDetail = false;
            if (customerByTableId != null) {
                if (coffeeTableByTableId.getStatus() == 1) {
                    updateTableIdOrderDetail = odi.updateTableIdOrderDetail(tableId, selectedTable.getTableId(), 0);
                } else {
                    updateTableIdOrderDetail = odi.updateTableIdOrderDetail(tableId, selectedTable.getTableId(), 2);
                }
            } else {
                updateTableIdOrderDetail = odi.updateTableIdOrderDetail(tableId, selectedTable.getTableId(), 0);
            }
            if (updateTableIdOrderDetail) {
                boolean updateStatusCoffeeTable = false;
                if (customerByTableId != null) {
                    if (coffeeTableByTableId.getStatus() == 2 && (customerByTableId.getStatus() == 0 || customerByTableId.getStatus() == 1)) {
                ...
                if (updateStatusCoffeeTable) {
                    boolean updateStatusCoffeeTableSwitch = false;
                    if (customerByTableId != null) {
                        if (coffeeTableByTableId.getStatus() == 1) {
                            updateStatusCoffeeTableSwitch = cti.updateStatusCoffeeTable(1, selectedTable.getTableId());
                        } else {
                            updateStatusCoffeeTableSwitch = cti.updateStatusCoffeeTable(2, selectedTable.getTableId());
                        }
                    } else {
                        updateStatusCoffeeTableSwitch = cti.updateStatusCoffeeTable(1, selectedTable.getTableId());
                    }
                    if (updateStatusCoffeeTableSwitch) {
                        sell.reload();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(SwitchTableJFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new SwitchTableJFrame().setVisible(true);
            }
        });
    }
